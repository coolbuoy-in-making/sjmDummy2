...
async def initialize_model():
    ...
    try:
        ...
        # Initialize the matching engine (which includes loading freelancers)
        await model.initialize_matching_engine()
        
        # Log available freelancers
...
def extract_freelancer_id(message: str) -> Optional[str]:
    """
    Extract freelancer ID from message.
    
    Args:
        message (str): freelancer message containing freelancer reference
        
    Returns:
        Optional[str]: Extracted freelancer ID or None if not found
    """
    # Look for freelancer ID patterns
    id_patterns = [
...
async def chat() -> Union[Dict, tuple]:
    try:
        ...
        if 'confirm_project_details' in message.lower() and project_details:
            ...
            return jsonify({
                ...
                'response': {
                    ...
                    'matchingProcess': {
                        ...
                        'searchStats': {
                            'totalFreelancers': len(model.freelancers),
        ...
        if message == 'confirm_project_details' and project_details:
            ...
            return jsonify({
                ...
                'response': {
                    'type': 'freelancerList',
                    'text': f"I found {len(matches)} matching freelancers:",
                    'freelancers': format_freelancer_matches(matches),
                    'matchingProcess': {
                        'steps': [
                            f"Analyzing {len(model.freelancers)} available freelancers",
...
def analyze_intent(message: str) -> str:
    """Analyze freelancer message intent"""
    intents = {
...
def format_freelancer_matches(matches: List[Dict]) -> List[Dict]:
    """Enhanced freelancer match formatting with more details"""
    return [{
...
def format_experience_details(freelancer) -> Dict[str, Any]:
    """Format freelancer's experience details"""
    return {
...
def format_availability(freelancer) -> Dict[str, Any]:
    """Format freelancer's availability status"""
    return {
